...
import org.cy.micoservice.blog.admin.provider.dao.SysAclModuleMapper;
import org.cy.micoservice.blog.admin.provider.dao.SysOrgMapper;
import org.cy.micoservice.blog.admin.provider.service.SysAclCoreService;
import org.cy.micoservice.blog.admin.provider.utils.acl.AclUtil;
import org.cy.micoservice.blog.admin.provider.utils.aclmodule.AclModuleUtil;
import org.cy.micoservice.blog.admin.provider.utils.orgUtil.LevelUtil;
import org.cy.micoservice.blog.admin.provider.utils.orgUtil.OrgUtil;
...
@Service
@DubboService
public class SysTreeFacadeImpl implements SysTreeFacade {
  ...
  public List<OrgLevelDto> orgTree() {
    ...
    return this.orgListToTree(dtoList);
  }
  ...
  private List<OrgLevelDto> orgListToTree(List<OrgLevelDto> dtoList) {
    ...
    // 从顶层开始递归生成组织树
    this.transformOrgTree(rootList, LevelUtil.ROOT, levelOrgMap);
    return rootList;
	}
  ...
  public List<AclModuleDto> roleAclTree(Long roleId) {
    ...
    List<SysAcl> userAclList = aclCoreService.getCurrentAdminAclList();
    ...
    List<SysAcl> roleAclList = aclCoreService.getRoleAclList(roleId);
  ...
  public List<AclModuleDto> adminAclTree(Long adminId) {
    List<SysAcl> userAclList = aclCoreService.getAdminAclList(adminId);
